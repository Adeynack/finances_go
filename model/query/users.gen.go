...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.Books = userHasManyBooks{
		...
		Owner: struct {
			...
			RelationField: field.NewRelation("Books.Owner", "model.User"),
			Books: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Books.Owner.Books", "model.Book"),
			},
		},
		ImportOrigins: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("Books.ImportOrigins", "model.ImportOrigin"),
		},
	}
...
func (a userHasManyBooks) Model(m *model.User) *userHasManyBooksTx {
	return &userHasManyBooksTx{a.db.Model(m).Association(a.Name())}
}

type userHasManyBooksTx struct{ tx *gorm.Association }

func (a userHasManyBooksTx) Find() (result []*model.Book, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyBooksTx) Append(values ...*model.Book) (err error) {
...
func (a userHasManyBooksTx) Replace(values ...*model.Book) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyBooksTx) Delete(values ...*model.Book) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
